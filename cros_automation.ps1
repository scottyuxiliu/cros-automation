...
function measurement {
    ...
    foreach ($i in 1..$loops) {
        Write-Verbose "preparing for $scenario scenario, loop $i ..."
        ...
        if ($scenario -eq "aquarium") {
            Write-Verbose "start atitool logging and launch $scenario ..."
            python.exe .\cros_automation.py atitool -p $test_system_ip_address -u $test_system_username -k $test_system_ssh_private_key_file -t $duration_aquarium -o "pm_log_$i.csv"
