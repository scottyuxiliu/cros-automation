...
class CrosDataLogger:
    ...
    def __init__(self, ip, test_system_username, ssh_private_key_file, debug):
        ...
        # for formatter, output function name too
        ...
        #     self.logger.error(
        #         f"paramiko authentication failed when connecting to {ip}. "
        #         f"it may be possible to retry with different credentials."
        #     )
        # except paramiko.SSHException:
        #     self.logger.error(
        #         f"paramiko ssh exception when connecting to {ip}. "
        #         f"there might be failures in SSH2 protocol negotiation or logic errors."
        #     )
    ...
    def __read_stdout(self, stdout):
        ...
        if stdout.channel.recv_exit_status() != 0:
            self.logger.error(
                f"{'(DEBUG MODE) ' if self.debug else ''}stdout.channel.recv_exit_status() returned {stdout.channel.recv_exit_status()}")
    ...
    def __exec_command(self, command, read_stdout=False, password=None):
        ...
        try:
            ...
            self.logger.error(
                "paramiko ssh exception. there might be failures in SSH2 protocol negotiation or logic errors.")

        else:
            if self.debug is True or read_stdout is True:
                stdout = self.__read_stdout(
                    stdout)  # if debug flag is true or read_stdout is true, capture stdout from exec_command. this is blocking.
                self.logger.info("command completed on the DUT")
            else:
                n = len(command.split(";"))  # get the number of commands that should be executed
                time.sleep(
                    n)  # exec_command does not work properly without this. every additional command requires one more second of wait time.
        ...
        if self.debug is True:
            try:
                stdin, stdout, stderr = self.ssh.exec_command(
                    f"cd /usr/local/atitool; ./atitool {arguments}")  # non-blocking call
                self.__read_stdout(stdout)
            except paramiko.SSHException:
                self.logger.error(
                    "paramiko ssh exception. there might be failures in SSH2 protocol negotiation or logic errors.")
            ...
            try:
                ...
                time.sleep(
                    1)  # exec_command does not work properly without this. every additional command requires one more second of wait time.
            except paramiko.SSHException:
                self.logger.error(
                    "paramiko ssh exception. there might be failures in SSH2 protocol negotiation or logic errors.")
            else:
    ...
    def atitool_log(self, duration, index, arguments, output_file_name):
        ...
        if index == 0:
            if arguments is None:
                self.logger.info(
                    f'execute: cd /usr/local/atitool; ./atitool -pmlogall -pmcount={int(duration * 1000 / AGT_PERIOD)} -pmperiod={AGT_PERIOD} -pmoutput="{output_file_name}"')
                self.__exec_command(
                    f'cd /usr/local/atitool; ./atitool -pmlogall -pmcount={int(duration * 1000 / AGT_PERIOD)} -pmperiod={AGT_PERIOD} -pmoutput="{output_file_name}"')
            else:
                self.logger.info(
                    f'execute: cd /usr/local/atitool; ./atitool -pmlogall -pmcount={int(duration * 1000 / AGT_PERIOD)} -pmperiod={AGT_PERIOD} -pmoutput="{output_file_name}" {arguments}')
                self.__exec_command(
                    f'cd /usr/local/atitool; ./atitool -pmlogall -pmcount={int(duration * 1000 / AGT_PERIOD)} -pmperiod={AGT_PERIOD} -pmoutput="{output_file_name}" {arguments}')
        else:
            if arguments is None:
                self.logger.info(
                    f'execute: cd /usr/local/atitool; ./atitool -i={index} -pmlogall -pmcount={int(duration * 1000 / AGT_PERIOD)} -pmperiod={AGT_PERIOD} -pmoutput="{output_file_name}"')
                self.__exec_command(
                    f'cd /usr/local/atitool; ./atitool -i={index} -pmlogall -pmcount={int(duration * 1000 / AGT_PERIOD)} -pmperiod={AGT_PERIOD} -pmoutput="{output_file_name}"')
            else:
                self.logger.info(
                    f'execute: cd /usr/local/atitool; ./atitool -i={index} -pmlogall -pmcount={int(duration * 1000 / AGT_PERIOD)} -pmperiod={AGT_PERIOD} -pmoutput="{output_file_name}" {arguments}')
                self.__exec_command(
                    f'cd /usr/local/atitool; ./atitool -i={index} -pmlogall -pmcount={int(duration * 1000 / AGT_PERIOD)} -pmperiod={AGT_PERIOD} -pmoutput="{output_file_name}" {arguments}')
    ...
    def agt_log(self, duration, index, arguments, output_file_name):
        ...
        if self.__exist_remote(f"{AGT_DIR_PATH}/agt"):
            if index == 0:
                if arguments is None:
                    self.logger.info(
                        f"{'(DEBUG MODE) ' if self.debug else ''}execute: cd /usr/local/agt; ./agt -pmlogall -pmcount={int(duration * 1000 / AGT_PERIOD)} -pmperiod={AGT_PERIOD} -pmoutput=\"{output_file_name}\"")
                    self.__exec_command(
                        f"cd /usr/local/agt; ./agt -pmlogall -pmcount={int(duration * 1000 / AGT_PERIOD)} -pmperiod={AGT_PERIOD} -pmoutput=\"{output_file_name}\"")
                else:
                    self.logger.info(
                        f"{'(DEBUG MODE) ' if self.debug else ''}execute: cd /usr/local/agt; ./agt -pmlogall -pmcount={int(duration * 1000 / AGT_PERIOD)} -pmperiod={AGT_PERIOD} -pmoutput=\"{output_file_name}\" {arguments}")
                    self.__exec_command(
                        f"cd /usr/local/agt; ./agt -pmlogall -pmcount={int(duration * 1000 / AGT_PERIOD)} -pmperiod={AGT_PERIOD} -pmoutput=\"{output_file_name}\" {arguments}")
            else:
                if arguments is None:
                    self.logger.info(
                        f"{'(DEBUG MODE) ' if self.debug else ''}execute: cd /usr/local/agt; ./agt -i={index} -pmlogall -pmcount={int(duration * 1000 / AGT_PERIOD)} -pmperiod={AGT_PERIOD} -pmoutput=\"{output_file_name}\"")
                    self.__exec_command(
                        f"cd /usr/local/agt; ./agt -i={index} -pmlogall -pmcount={int(duration * 1000 / AGT_PERIOD)} -pmperiod={AGT_PERIOD} -pmoutput=\"{output_file_name}\"")
                else:
                    self.logger.info(
                        f"{'(DEBUG MODE) ' if self.debug else ''}execute: cd /usr/local/agt; ./agt -i={index} -pmlogall -pmcount={int(duration * 1000 / AGT_PERIOD)} -pmperiod={AGT_PERIOD} -pmoutput=\"{output_file_name}\" {arguments}")
                    self.__exec_command(
                        f"cd /usr/local/agt; ./agt -i={index} -pmlogall -pmcount={int(duration * 1000 / AGT_PERIOD)} -pmperiod={AGT_PERIOD} -pmoutput=\"{output_file_name}\" {arguments}")
        else:
    ...
    def agt_internal_log(self, duration, index, arguments, output_file_name):
        ...
        if self.__exist_remote(F"{AGT_INTERNAL_DIR_PATH}/agt_internal"):
            ...
            if index == 0:
                if arguments is None:
                    self.logger.info(
                        f"{'(DEBUG MODE) ' if self.debug else ''}execute: cd /usr/local/agt_internal; ./agt_internal -pmlogall -pmcount={int(duration * 1000 / AGT_PERIOD)} -pmperiod={AGT_PERIOD} -pmoutput=\"{output_file_name}\"")
                    self.__exec_command(
                        f"cd /usr/local/agt_internal; ./agt_internal -pmlogall -pmcount={int(duration * 1000 / AGT_PERIOD)} -pmperiod={AGT_PERIOD} -pmoutput=\"{output_file_name}\"")
                else:
                    self.logger.info(
                        f"{'(DEBUG MODE) ' if self.debug else ''}execute: cd /usr/local/agt_internal; ./agt_internal -pmlogall -pmcount={int(duration * 1000 / AGT_PERIOD)} -pmperiod={AGT_PERIOD} -pmoutput=\"{output_file_name}\" {arguments}")
                    self.__exec_command(
                        f"cd /usr/local/agt_internal; ./agt_internal -pmlogall -pmcount={int(duration * 1000 / AGT_PERIOD)} -pmperiod={AGT_PERIOD} -pmoutput=\"{output_file_name}\" {arguments}")
            else:
                if arguments is None:
                    self.logger.info(
                        f"{'(DEBUG MODE) ' if self.debug else ''}execute: cd /usr/local/agt_internal; ./agt_internal -i={index} -pmlogall -pmcount={int(duration * 1000 / AGT_PERIOD)} -pmperiod={AGT_PERIOD} -pmoutput=\"{output_file_name}\"")
                    self.__exec_command(
                        f"cd /usr/local/agt_internal; ./agt_internal -i={index} -pmlogall -pmcount={int(duration * 1000 / AGT_PERIOD)} -pmperiod={AGT_PERIOD} -pmoutput=\"{output_file_name}\"")
                else:
                    self.logger.info(
                        f"{'(DEBUG MODE) ' if self.debug else ''}execute: cd /usr/local/agt_internal; ./agt_internal -i={index} -pmlogall -pmcount={int(duration * 1000 / AGT_PERIOD)} -pmperiod={AGT_PERIOD} -pmoutput=\"{output_file_name}\" {arguments}")
                    self.__exec_command(
                        f"cd /usr/local/agt_internal; ./agt_internal -i={index} -pmlogall -pmcount={int(duration * 1000 / AGT_PERIOD)} -pmperiod={AGT_PERIOD} -pmoutput=\"{output_file_name}\" {arguments}")
    ...
    def pac_log(self, duration, password):
        ...
        self.logger.info("--------------------------------------------------------------------------------")
        self.logger.info(
            f"{'(DEBUG MODE) ' if self.debug else ''}execute: sudo -S sh -c \"cd ~/pac-db; python3 pac_db2.py -c guybrush_r0_pacs_mainsmt.py -t {duration}\"")
        self.__exec_command(
            f"sudo -S sh -c \"cd ~/pac-db; python3 pac_db2.py -c guybrush_r0_pacs_mainsmt.py -t {duration}\"", True,
            password)
